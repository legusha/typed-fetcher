...
describe('ErrorManager', () => {
    ...
    describe('parse', () => {
        it('should return HttpResponseFull with error if input is HttpError', () => {
            ...
            const result = errorManager.parse(httpError);
        ...
        it('should rethrow if input is not HttpError', () => {
            ...
            expect(() => errorManager.parse(regularError)).toThrow(regularError);
        });
    });
    ...
    describe('isHttpError', () => {
        it('should return true for ErrorJSON', () => {
            ...
            expect(ErrorManager.isHttpError(error)).toBe(true);
        });

        it('should return false for regular Error', () => {
            const error = new Error('test');
            expect(ErrorManager.isHttpError(error)).toBe(false);
        });

        it('should return false for plain object without status', () => {
            expect(ErrorManager.isHttpError({ message: 'test' })).toBe(false);
        });

        it('should return true for plain object with message and status', () => {
            expect(ErrorManager.isHttpError({ message: 'test', status: 400 })).toBe(true);
        });
    });
});
